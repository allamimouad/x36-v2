...
- **FileSystemApi takes nodes, not primitives**: every method accepts full `FolderNode` / `FileSystemNode` values (e.g. `move(node, newParent)`), not decomposed ids and paths. Reason: the SharePoint adapter will be a thin shim over an auto-generated client built from the SharePoint OpenAPI spec — that client's signatures are dictated externally. Passing nodes gives the adapter access to every field it might need to map onto the generated DTOs without forcing our internal API to grow whenever the wire spec evolves. Mock reads `arg.id` to look up its canonical copy; adapter reads whichever combination of `id`, `path`, `name`, `parent.path`, etc. the generated client expects. No id→url cache anywhere.
